...
|---------|-------------|
| `claudbot` | Interactive menu — start, resume, recall, dashboard, dream… |
| `claudbot start` | Launch the agent directly, skipping the menu |
...
Summaries are kept fresh **automatically**: every `claudbot` launch kicks off a
detached background indexer (`node memory.mjs index`), and another pass runs the
moment a session ends — so the "Last session" banner and menu card always show a
real LLM summary, not a heuristic. Summaries are generated by
`CLAUDBOT_SUMMARY_AGENT` (default `fast` → Nemotron Nano) and cached in
...
An overnight idle process curates AI/ML/dev news *to learn from* (not just skim),
and a local web "command center" surfaces it alongside your projects each morning.

```bash
claudbot briefing            # build one digest now
claudbot briefing --watch    # rebuild on a schedule (CLAUDBOT_BRIEFING_INTERVAL_H, default 6h)
claudbot dashboard           # open http://localhost:4500
```

- **Briefing** pulls free, no-auth sources (Hacker News, arXiv, Reddit, RSS),
  dedups, ranks for learning value against `briefing/interests.yaml`, then has the
  `fast` agent write a learning card per top item (why it matters, the key
  takeaway, difficulty, read time). Output: `briefing/data/latest.json`. Runs
  headless on NIM only — no Claude Code needed, so it can live on the NUC.
