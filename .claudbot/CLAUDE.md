...
8. **Desktop widgets** (`claudbot widgets`): four Rainmeter widgets on the Windows desktop — status/launcher, post-it, stock watchlist, Notion tasks. Skins never touch the network; `widgets/bridge.mjs` feeds them flat text files. Desktop-only, not on the portable drive. Protocol: `docs/widgets.md`.
9. **Portable drive** (`npm run make-portable -- --target <drive>`): the whole assistant on a USB stick — code, bundled Node, Claude Code CLI, and all personal data encrypted at rest. Runs on any Windows/macOS/Linux host with nothing installed and leaves nothing behind. Protocol: `docs/portable.md`.
